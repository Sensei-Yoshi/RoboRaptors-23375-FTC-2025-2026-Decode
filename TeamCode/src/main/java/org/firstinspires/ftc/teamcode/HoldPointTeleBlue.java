...
@Config
@TeleOp(name = "Test Tele Blue", group = "Tuning")
public class HoldPointTeleBlue extends LinearOpMode {
    ...
    // ─────────────────────────────────────────────
    //  Heading lock  (uses Pedro's FollowerConstants)
    // ─────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────
    //  Translational lock  (uses Pedro's FollowerConstants)
    // ─────────────────────────────────────────────
    private PIDFController translationalPidX;
    private PIDFController translationalPidY;
    private double         lockedX                 = 0.0;
    private double         lockedY                 = 0.0;
    private boolean        translationalLockActive = false;
    ...
    public void runOpMode() {
        ...
        headingPid         = new PIDFController(follower.constants.coefficientsHeadingPIDF);
        translationalPidX  = new PIDFController(follower.constants.coefficientsTranslationalPIDF);
        translationalPidY  = new PIDFController(follower.constants.coefficientsTranslationalPIDF);
        ...
        while (opModeIsActive()) {
            // Keep Pedro localizer ticking — no movement vectors sent to Pedro
            follower.update();
            ...
            // ── AIMING state: aim PIDF has full yaw authority ──
            if (autoState == AutoState.AIMING) {
                if (cameraBlocked) {
                    ...
                    double yawLimit = (distance > LONG_RANGE_AIM_THRESHOLD)
                            ? AIM_MAX_YAW_LONG_RANGE : 1.0;
                    yaw = clamp(rawYaw, -yawLimit, yawLimit);

                    if (Math.abs(error) < aimTolerance) {
                        // Re-snapshot heading once aiming has settled, then hand off to heading lock
                        lockedHeading     = follower.getPose().getHeading();
                        headingPid        = new PIDFController(follower.constants.coefficientsHeadingPIDF);
                        headingLockActive = true;
                        // Snapshot X/Y now that we are settled on target, enable translational lock
                        lockedX                 = follower.getPose().getX();
                        lockedY                 = follower.getPose().getY();
                        translationalPidX       = new PIDFController(follower.constants.coefficientsTranslationalPIDF);
                        translationalPidY       = new PIDFController(follower.constants.coefficientsTranslationalPIDF);
                        translationalLockActive = true;
                        yaw               = 0;
                        axial             = 0;
                        lateral           = 0;
            ...
            // ── Heading lock: holds heading during SPINNING and FIRING ──
            // Aim PIDF controls yaw during AIMING so heading lock is skipped then
            if (headingLockActive && autoState != AutoState.AIMING) {
                headingPid.setCoefficients(follower.constants.coefficientsHeadingPIDF);
                headingPid.updateError(getHeadingError());
                yaw = clamp(-headingPid.run(), -1.0, 1.0);
            }

            // ── Translational lock: holds X/Y position during SPINNING and FIRING ──
            // Skipped during AIMING — driver has free translational control while aiming
            if (translationalLockActive && autoState != AutoState.AIMING) {
                translationalPidX.setCoefficients(follower.constants.coefficientsTranslationalPIDF);
                translationalPidY.setCoefficients(follower.constants.coefficientsTranslationalPIDF);
                ...
                // (axial = robot forward, lateral = robot strafe)
                double robotAxial   =  errorX * Math.cos(heading) + errorY * Math.sin(heading);
                double robotLateral = -errorX * Math.sin(heading) + errorY * Math.cos(heading);

                translationalPidX.updateError(robotAxial);
                translationalPidY.updateError(robotLateral);

                axial   = clamp(translationalPidX.run(), -1.0, 1.0);
                lateral = clamp(-translationalPidY.run(), -1.0, 1.0);
            }

            // Always drive with our own mecanum — Pedro is localizer only
    ...
    private void initHardware() {
        ...
        // ── Lift motor setup — no power until Y is pressed ──
        ...
        // ── Pedro follower — localizer only, no teleop drive vectors used ──
    ...
    private double startShootSequence(double yaw) {
        if (manualOverride) {
            ...
            // Snapshot heading — heading lock active immediately in manual mode
            lockedHeading     = follower.getPose().getHeading();
            headingLockActive = true;
            headingPid        = new PIDFController(follower.constants.coefficientsHeadingPIDF);
            // Snapshot X/Y — translational lock active immediately in manual mode
            lockedX                 = follower.getPose().getX();
            lockedY                 = follower.getPose().getY();
            translationalPidX       = new PIDFController(follower.constants.coefficientsTranslationalPIDF);
            translationalPidY       = new PIDFController(follower.constants.coefficientsTranslationalPIDF);
            ...
            // Heading lock and translational lock are NOT active yet —
            // aim PIDF controls yaw during AIMING.
            // Both will be enabled once aiming settles in the AIMING block above.
            headingLockActive       = false;
            translationalLockActive = false;
            aimPid             = new PIDFController(new PIDFCoefficients(Kp_aim, Ki_aim, Kd_aim, Kf_aim));
    ...
    // =========================================================
    //  Drive  (always used — Pedro is localizer only)
    // =========================================================
    ...
    // =========================================================
    //  Heading lock helper
    //  Uses same MathFunctions pattern as Pedro autonomous
    // =========================================================
    private double getHeadingError() {
        double heading = follower.getPose().getHeading();
        return MathFunctions.getTurnDirection(heading, lockedHeading)
    ...
    private void buildLookupTables() {
        ...
        controlPointsRPM.add(60,  1150);
        controlPointsRPM.add(65,  1150);
        ...
        controlPointsRPM.add(120, 1400);
        controlPointsRPM.add(125, 1400);
        ...
        controlPointsHood.add(120, 0.539);
        controlPointsHood.add(125, 0.539);
    ...
    private void updateTelemetry(double targetRPM, double hoodTarget, double yaw) {
        ...
        telemetry.addData("Pose/HeadingDeg",         Math.toDegrees(follower.getPose().getHeading()));
